...
export default function RegisterPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    registerMutation.mutate(
      ...
      {
        onSuccess: (data: any) => {
          toast.success(data?.message || "Inscription réussie ! Veuillez vérifier votre email.");
  ...
  async function handleGoogleSuccess(credentialResponse: any) {
    try {
      ...
      if (res.ok && data.token && data.user) {
        ...
        toast.error(data.message || "Erreur d'inscription Google");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 bg-[#003B8F] relative items-center justify-center">
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 70% 30%, #65BDF2 0%, transparent 50%)" }} />
        <div className="relative text-center text-white px-12">
          <Link href="/" className="inline-flex items-center justify-center mb-6">
            <img src="/logo.png" alt="Sparkle News" className="h-24 w-auto object-contain" />
          </Link>
          <h2 className="text-4xl font-black mb-4">Rejoignez-nous</h2>
          <p className="text-white/70 text-lg leading-relaxed">
            Créez votre compte pour rester connecté à l’actualité et profiter d’une expérience personnalisée.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-background">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-black text-[#003B8F] dark:text-white mb-2">Inscription</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Votre nom" required className="pl-10" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="8 caractères minimum" required minLength={8} className="pl-10 pr-10" />
            ...
            <Button type="submit" className="w-full sparkle-gradient text-white border-0 hover:opacity-90 h-11 text-base font-semibold" disabled={registerMutation.isPending}>
          ...
          <div className="mt-6 flex justify-center">
            <GoogleLogin
              ...
              onError={() => toast.error("L'inscription avec Google a échoué.")}
              theme="outline"
              size="large"
              shape="pill"
            />
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-muted-foreground">
              Déjà inscrit ?{" "}
              <Link href="/connexion" className="text-[#006FE6] hover:text-[#003B8F] font-medium transition-colors">Se connecter</Link>
            </p>
          </div>
        </div>
      </div>
